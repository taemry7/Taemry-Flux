...
// Permanent data preservation: Never purge user accounts, deposits, packages or balances
function ensureDataPreserved() {
  try {
    const db = getDb();
    if (db && typeof db._persist === 'function') {
      db._persist();
    }
  } catch (err) {
    console.warn('[Data Preservation Notice]:', err.message);
  }
}
ensureDataPreserved();
