/**
 * TAEMRY FLUX - Milestone Definitions & Computation Logic
 * Centralized milestone ladder for Personal & Team Ads.
 */
...
/**
 * Calculates current progression, next target, and claimable rewards
 * for personal or team milestones.
 */
export const calculateMilestoneStatus = (currentAds = 0, milestonesList = [], claimedArray = []) => {
  const claimedSet = new Set(claimedArray.map(Number));

  // Find next milestone that is greater than current ads
  const nextMilestone = milestonesList.find((m) => m.ads > currentAds) || milestonesList[milestonesList.length - 1];

  // Find lowest achieved milestone that has NOT been claimed yet
  const nextClaimable = milestonesList.find((m) => currentAds >= m.ads && !claimedSet.has(m.ads));

  // Calculate percentage towards next milestone
  const targetAds = nextMilestone ? nextMilestone.ads : 1000;
  const progressPercentage = Math.min(100, Math.round((currentAds / targetAds) * 100));

  return {
    currentAds: Number(currentAds),
    nextMilestone: {
